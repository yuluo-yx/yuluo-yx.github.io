...
description: 了解如何在 Spring AI Alibaba 中实现 Multi-agent 协作，包括工具调用和交接模式
date: 2025-12-28
tags: [Spring AI Alibaba, ReactAgent, Multi-agent, Agent Tool, Tool Calling]
...
| 模式 | 工作原理 | 控制流 | 使用场景 |
| ---- | -------- | ------ | -------- |
| **Tool Calling** | Supervisor Agent将其他Agent作为*工具*调用。"工具"Agent不直接与用户对话——它们只执行任务并返回结果。 | 集中式：所有路由都通过调用Agent。 | 任务编排、结构化工作流。 |
...
关于工具调用模式的使用请查看 [HandOffs 模式文档](./03-Multi-Agent.md)。
...
ReactAgent writerAgent = ReactAgent.builder()
    ...
    .instruction("根据结构化输入（topic、wordCount、style）创作文章，并返回结构化输出。")
    .inputType(ArticleRequest.class)
...
ReactAgent summarizerAgent = ReactAgent.builder()
    .name("summarizer_agent").model(chatModel)
...
> **提示**：为每个子Agent提供清晰、准确的 \`description\` 非常重要，这直接影响主Agent如何选择合适的工具。`;export{n as default};
